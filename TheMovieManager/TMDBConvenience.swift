...
extension TMDBClient {
    ...
    */
    ...
    func getWatchlistMovies(_ completionHandlerForWatchlist: @escaping (_ result: [TMDBMovie]?, _ error: NSError?) -> Void) {
        ...
        let _ = taskForGETMethod(mutableMethod, parameters: [TMDBClient.ParameterKeys.SessionID: self.sessionID! as AnyObject]) { (results, error) in
            
            print("Getting watch list result: \(results) error: \(error)")
            ...
            completionHandlerForWatchlist(TMDBMovie.moviesFromResults(results), nil)
    ...
    func postToWatchlist(_ movie: TMDBMovie, watchlist: Bool, completionHandlerForWatchlist: @escaping (_ result: Int?, _ error: NSError?) -> Void) {
        ...
        let _ = taskForPOSTMethod(method, parameters: parameters as [String : AnyObject], jsonBody: jsonBody) { (result, error) in
        
            print("Post to watchlist result: \(result) error: \(error)")
            ...
            guard let result = result?[TMDBClient.JSONResponseKeys.StatusCode] as? Int else {
            ...
            completionHandlerForWatchlist(result, nil)
        }
    }
}
